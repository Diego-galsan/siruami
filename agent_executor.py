...
class KarleyAgentExecutor(AgentExecutor):
    ...
    async def _process_request(
        ...
        async for event in self._run_agent(session_id, new_message):
            if event.is_final_response():
                ...
                await task_updater.add_artifact(parts) #task_updater.add_artifact(parts)
                await task_updater.complete()#task_updater.complete()
                break
    ...
    async def execute(
        ...
        if not context.current_task:
            await updater.submit()#updater.submit()
        await updater.start_work()#updater.start_work()
